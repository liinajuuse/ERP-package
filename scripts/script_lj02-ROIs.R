...
ERP.test <- subset(oz, (Emotion == "Happy" | Emotion == "Neutral"))
...
time_pt = time_pt[100:400]
T <- length(time_pt)                 
n <- nrow(ERP.test)

#löön lahku metadata ja erpdta

covariates = ERP.test[, 1:9]
erpdta = ERP.test[, -(1:9)] 
with(covariates, table(Electrode, Emotion, Condition))
with(covariates, table(ROI, Emotion, Condition))
...
covariates$Condition = factor(covariates$Condition, c("Ekman", "Under"))
covariates$Emotion = factor(covariates$Emotion, c("Happy", "Neutral"))

covariates = droplevels(covariates)

channels <- levels(covariates$ROI)
...
for (i in 1:2) {
  for (j in 1:2) {
    select <- (covariates$ROI == channels[i]) & 
...
design <- model.matrix(~ C(Subj, sum) + Condition + Emotion + 
                         ROI + ROI:Emotion + ROI:Condition +
                         Emotion:Condition + ROI:Emotion:Condition, 
                       data = covariates)
colnames(design)[c(1, 117:120)]
...
for (i in 1:2) {
  for (j in 1:2) {
    ...
    covariates$ROI <- relevel(covariates$ROI, ref = channels[i])
    design <- model.matrix(~ C(Subj, sum) + Condition + Emotion + 
                             ROI + ROI:Emotion + ROI:Condition +
                             Emotion:Condition + ROI:Emotion:Condition, 
                           data = covariates)
    erpplot(erpdta, design, effect = effect, interval = "simultaneous", 
            nbs = 20, lwd = 2, frames = time_pt,
            xlab = "Time (ms)", ylab = "Condition effect")
    title(paste("Group ", groups[j], " ROI ", channels[i], sep = ""))
  }
}
...
design0 <- model.matrix(~ C(Subj, sum) + Condition + Emotion + 
                          ROI + ROI:Condition + Emotion:Condition, 
                        data = covariates)
erpFtest(erpdta, design, design0, nbf = F$nbf)$pval
...
avetest <- erpavetest(erpdta.o2e, design, design0)
...
erpplot(erpdta.o2e, design, effect = ncol(design), lwd = 2, 
        interval = "pointwise", frames = time_pt, nbs = 40,
