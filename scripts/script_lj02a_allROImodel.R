...
library(tidyverse)
library(ERP)
load("C:/Users/liina/OneDrive/PhD/EMO2021/EMO2021/ERP-package/output/data-threshold6-ROI.RData")

ERP.test <- oz
ERP.test = na.omit(ERP.test)
ERP.test = ERP.test[, 1:408] # metadata + 600 ms of values
rownames(ERP.test) <- NULL # reset row names 
...
time_pt = time_pt[1:400] # 600 ms timeline
T <- length(time_pt)                 
n <- nrow(ERP.test)

# dividing metadata and erpdta (time intervals)
